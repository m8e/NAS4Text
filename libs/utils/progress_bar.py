...
def build_progress_bar(hparams, iterator, epoch=None, prefix=None, default='tqdm', no_progress_bar='none'):
    if hparams.log_format is None:
        hparams.log_format = no_progress_bar if hparams.no_progress_bar else default

    if hparams.log_format == 'tqdm' and not sys.stderr.isatty():
        hparams.log_format = 'simple'

    if hparams.log_format == 'json':
        bar = JSONProgressBar(iterator, epoch, prefix, hparams.log_interval)
    elif hparams.log_format == 'none':
        bar = NoOpProgressBar(iterator, epoch, prefix)
    elif hparams.log_format == 'simple':
        bar = SimpleProgressBar(iterator, epoch, prefix, hparams.log_interval)
    elif hparams.log_format == 'tqdm':
        bar = TqdmProgressBar(iterator, epoch, prefix)
    else:
        raise ValueError('Unknown log format: {}'.format(hparams.log_format))
    return bar
...
class JSONProgressBar(ProgressBar):
